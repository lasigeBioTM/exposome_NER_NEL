...
def build_information_content_dict(candidates_dir, id_to_info, mode=None):
    ...
    for term_id in term_counts:        
        ...
        if mode == 'extrinsic':
            ...
            #TODO: correct number of descendants
            try:
                #num_descendants = len(nx.descendants(kb_graph, term_id))
                num_descendats = id_to_info[term_id][2]
                term_probability = (num_descendants + 1) / total_terms
                
            except:
                #TODO: solve error: etworkx.exception.NetworkXError: 
                # The node GO_0016265 is not in the graph
