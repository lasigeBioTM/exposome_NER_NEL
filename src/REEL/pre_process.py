...
from src.REEL.utils import entity_string, check_if_candidates_dir
sys.path.append("./")
...
def build_entity_candidate_dict(
    ...
    """Build a dictionary including the candidates for all entity mentions in all 
    the input documents.
    ...
    message= 'Pre-processing documents ({})'.format(kb)
    ...
    for doc in ner_out_docs:
        ...
        if doc_entities_final != []:
            ...
            #entities_candidates[doc_id] = doc_entities_final
...
def pre_process(run_id, ner_dir, kb, entity_type, link_mode, nil_mode, abbreviations):
    ...
    # Load preprocessed dicts and networkx graph
    name_to_id = {}
    synonym_to_id = {}
    #kb_graph = None
