...
import psutil
import gc
from src.classes import Document, Sentence, Entity
        ...
        for ent_type in types:
            ...
            if ent_type != '':
                assert ent_type in available_kbs, \
                    ...
                    "phenotype"'.format(ent_type)
                
                given_kb = types[ent_type]
                
                assert given_kb in available_kbs[ent_type], \
                    '{} is an invalid KB for entity type "{}"! Options:\
                    {}'.format(given_kb, ent_type, str(available_kbs[ent_type]))          
...
def check_input_args(recognize, link, types, input_format, input_text, 
    ...
    assert recognize == True or link == True, \
        ...
        Linking (NEL). Options: "recognize" (NER), "link" (NEL). Set \
        one of both to "True"'
    ...
    if recognize:
        ...
        if input_text == None and in_dir == None:
            ...
            directory containing file(s) with text to annotate. \
            Set the arguments "input_text" or "text_dir"')
        
        elif input_text != None and txt != None:
            raise ValueError('It is necessary to input either a text OR a \
            directory containing file(s) with text to annotate, not both. \
            Set the arguments "input_text" or "text_dir"')
    ...
    if link:
        ...
        if not recognize:
            ...
            assert ner_dir != None, 'It is necessary to specfiy the directory \
                containing the ouput from the NER stage. Set the argument \
                "ner_dir" and "ner_format"'
...
def parse_pubtator_file(filename):
    ...
    with open(corpus_dir + filename, 'r') as corpus_file:
        ...
        for line in data:
            ...
            if len(line_data) == 6:
                ...
                parsed_data[doc_id][1].append(annotation)
...
def parse_bioc_xml_file(filename):
                    ...
                    for subelement2 in subelement:                                
                        ...
                        if subelement2.tag == "annotation":
                            ...
                            for subelement3 in subelement2: 
                                ...
                                elif subelement3.tag == "locations":
                                    
                                    if subelement3.attrib["key"] == "offset":
                                        pass 
                                    
                                    if subelement3.attrib["key"] == "length":
                                        pass

                                #text = annot['text']
                                #begin = annot['locations']['offset']
                                #end = str(int(begin) + int(annot['locations']['length']))

                                if entity_text != '':
                                    annotation = (kb_id, entity_text)
                                    annotation = (ent_type, begin, end, text)
                    a               
                                #parsed_data[doc_id][1].append(annotation)
...
def sentence_splitter(doc_text, lang_model):
    ...
    :param texts: text to be splitted
    :type texts: str
...
def objectify_entities(entities, sent_pos, includes_links=False):
    """Convert inputted recognized entities into objects of types Entity.
...
def import_reel_results(doc_id, run_ids):
    """Parse the results outputted by REEL2 for given document from the file 
    into a dictionary.
    ...
    for run_id in run_ids:
        ...
        if linked_entities_type != {}:
            ...
            linked_entities = merge_dicts(linked_entities, linked_entities_type_up)
...
def garbage_collect(threshold=50.0):
    ...
    auto_garbage_collection - Call the garbage collection if memory used is greater than 80% of total available memory.
        ...
        threshold - Default value of 80%.  Amount of memory in use that triggers the garbage collection call.
    """
