...
def parse_Ab3P_output(filepaths):
    ...
    for filepath in filepaths:
        ...
        #doc_id = filepath.strip('tmp/').strip('.txt')
        filepath_up ='src/abbreviation_detector/Ab3P/tmp/{}_abbrvs'.format(doc_id)
...
def run_Ab3P(input_dir):
    ...
    os.chdir('src/abbreviation_detector/Ab3P/')
    ...
    for filepath in filepaths:
        ...
        if '.txt' in filepath or '.ann' in filepath:
            doc_id = filepath[:-4]#.strip('.txt').strip('.ann')
    ...
    return parse_Ab3P_output(filepaths) 